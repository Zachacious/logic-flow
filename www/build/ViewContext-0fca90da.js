...
class ViewContext {
    ...
    updateVisibleElements() {
        ...
        const visibleNodes = this.viewportQuadtree.query({
            ...
            width: rect.width,
            height: rect.height,
        }, [], this.camera.pos, this.camera.zoom);
        this.prevVisibleElements = this.visibleElements;
        this.visibleElements = visibleNodes.map((node) => node.id);
        ...
        for (const id of allElements) {
            const el = document.getElementById(id);
            if (el) {
                el.setAttribute('data-visible', this.visibleElements.includes(id) ? 'visible' : 'hidden');
            }
        }
...
export { ViewContext as V, throttle as t };

//# sourceMappingURL=ViewContext-0fca90da.js.map